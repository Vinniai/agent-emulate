...
```bash
# Vercel only
npx agent-emulate --service vercel
...
curl -X PATCH http://localhost:4000/v2/user \
  ...
  -d '{"name": "New Name", "email": "new@agent-emulate.dev"}'
...
curl -X POST "http://localhost:4000/v2/teams/team_id/members" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"email": "dev@agent-emulate.dev", "role": "MEMBER"}'
```
...
curl -X POST http://localhost:4000/v10/projects/my-app/domains \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "agent-emulate.dev", "redirect": null, "redirectStatusCode": null, "gitBranch": null}'
...
curl http://localhost:4000/v9/projects/my-app/domains/agent-emulate.dev \
  -H "Authorization: Bearer $TOKEN"

# Verify domain
curl -X POST http://localhost:4000/v9/projects/my-app/domains/agent-emulate.dev/verify \
  -H "Authorization: Bearer $TOKEN"
```
