...
```bash
# Resend only
npx agent-emulate --service resend
...
curl -X POST http://localhost:4000/emails \
  ...
  -d '{"from": "hello@agent-emulate.dev", "to": "user@agent-emulate.dev", "subject": "Hello", "html": "<p>Hi</p>"}'

# Send batch (up to 100)
curl -X POST http://localhost:4000/emails/batch \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '[{"from": "a@agent-emulate.dev", "to": "b@agent-emulate.dev", "subject": "One", "html": "<p>1</p>"}]'
...
curl -X POST http://localhost:4000/domains \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "agent-emulate.dev", "region": "us-east-1"}'
...
curl -X POST http://localhost:4000/audiences/<audience_id>/contacts \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"email": "user@agent-emulate.dev", "first_name": "Jane", "last_name": "Doe"}'
...
curl -X POST $BASE/emails \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"from": "auth@agent-emulate.dev", "to": "user@agent-emulate.dev", "subject": "Your code", "html": "<p>Code: <strong>482910</strong></p>"}'
