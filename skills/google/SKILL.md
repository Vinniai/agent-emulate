...
allowed-tools: Bash(npx agent-emulate:*), Bash(agent-emulate:*), Bash(curl:*)
---
...
```bash
# Google only
npx agent-emulate --service google
...
Google Workspace accounts include an `hd` claim in ID tokens and userinfo responses identifying the user's hosted domain. The emulator derives this automatically from the user's email domain. Consumer domains (`agent-emulate.dev`, `googlemail.com`) omit the claim, matching real Google behavior.
...
curl -X POST http://localhost:4002/gmail/v1/users/me/messages/send \
  ...
  -d '{"to": "someone@agent-emulate.dev", "subject": "Hello", "body_text": "Hi there"}'

# Insert message (bypass send)
curl -X POST http://localhost:4002/gmail/v1/users/me/messages \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"to": "test@agent-emulate.dev", "from": "me@agent-emulate.dev", "subject": "Test", "body_text": "Body", "labelIds": ["INBOX"]}'

# Import message
curl -X POST http://localhost:4002/gmail/v1/users/me/messages/import \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"to": "test@agent-emulate.dev", "from": "external@agent-emulate.dev", "subject": "Imported", "body_text": "Content"}'
...
curl -X POST http://localhost:4002/gmail/v1/users/me/drafts \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"message": {"to": "someone@agent-emulate.dev", "subject": "Draft subject", "body_text": "Draft body"}}'
...
curl -X POST http://localhost:4002/gmail/v1/users/me/settings/filters \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"criteria": {"from": "alerts@agent-emulate.dev"}, "action": {"addLabelIds": ["Label_ops"]}}'
...
curl -X POST http://localhost:4002/calendar/v3/calendars/primary/events \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"summary": "Team Meeting", "start": {"dateTime": "2025-01-10T14:00:00Z"}, "end": {"dateTime": "2025-01-10T15:00:00Z"}, "attendees": [{"email": "dev@agent-emulate.dev"}]}'
...
curl -X POST $BASE/gmail/v1/users/me/messages/send \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"to": "someone@agent-emulate.dev", "subject": "Test", "body_text": "Hello"}'
