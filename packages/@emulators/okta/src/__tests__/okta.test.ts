...
function createTestApp() {
  ...
  tokenMap.set("mgmt-token", { login: "admin@agent-emulate.dev", id: 1, scopes: ["okta.*"] });
  tokenMap.set("ssws-token", { login: "admin@agent-emulate.dev", id: 1, scopes: ["okta.*"] });
  ...
  seedFromConfig(store, base, {
    users: [{ login: "alice@agent-emulate.dev", first_name: "Alice", last_name: "Example" }],
  ...
  describe("Users API", () => {
    ...
    it("creates STAGED user with activate=false", async () => {
      const res = await app.request(`${base}/api/v1/users?activate=false`, {
        ...
        body: JSON.stringify({
          profile: {
            login: "newuser@agent-emulate.dev",
            email: "newuser@agent-emulate.dev",
            firstName: "New",
            lastName: "User",
          },
        }),
    ...
    it("replaces and partially updates user profile", async () => {
      const create = await app.request(`${base}/api/v1/users`, {
        ...
        body: JSON.stringify({
          profile: {
            login: "replace@agent-emulate.dev",
            email: "replace@agent-emulate.dev",
            firstName: "Replace",
            lastName: "Me",
          },
        }),
      ...
      const put = await app.request(`${base}/api/v1/users/${userId}`, {
        ...
        body: JSON.stringify({
          profile: {
            login: "replace2@agent-emulate.dev",
            email: "replace2@agent-emulate.dev",
            firstName: "Replaced",
            lastName: "User",
          },
        }),
      ...
      expect((putBody.profile as Record<string, unknown>).login).toBe("replace2@agent-emulate.dev");
    ...
    it("runs lifecycle actions", async () => {
      const create = await app.request(`${base}/api/v1/users`, {
        ...
        body: JSON.stringify({
          profile: {
            login: "life@agent-emulate.dev",
            email: "life@agent-emulate.dev",
            firstName: "Life",
            lastName: "Cycle",
          },
        }),
    ...
    it("supports pagination with Link header", async () => {
      for (let i = 0; i < 3; i += 1) {
        await app.request(`${base}/api/v1/users`, {
          ...
          body: JSON.stringify({
            profile: {
              login: `page-${i}@agent-emulate.dev`,
              email: `page-${i}@agent-emulate.dev`,
              firstName: "Page",
              lastName: String(i),
            },
          }),
    ...
    it("deactivates first, then deletes on second request", async () => {
      const create = await app.request(`${base}/api/v1/users`, {
        ...
        body: JSON.stringify({
          profile: {
            login: "delete@agent-emulate.dev",
            email: "delete@agent-emulate.dev",
            firstName: "Delete",
            lastName: "Me",
          },
        }),
  ...
  describe("seed from config", () => {
    it("seeds users, groups, apps, oauth clients, auth servers and deduplicates", () => {
      ...
      seedFromConfig(seedStore, base, {
        users: [
          { login: "config-user@agent-emulate.dev", first_name: "Config", last_name: "User" },
          { login: "config-user@agent-emulate.dev", first_name: "Config", last_name: "User" },
        ],
      ...
      seedFromConfig(seedStore, base, {
        users: [{ login: "config-user@agent-emulate.dev", first_name: "Config", last_name: "User" }],
      });

      const okta = getOktaStore(seedStore);
      expect(okta.users.findBy("login", "config-user@agent-emulate.dev")).toHaveLength(1);
