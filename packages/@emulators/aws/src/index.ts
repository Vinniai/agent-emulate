...
import { secretsManagerRoutes } from "./routes/secretsmanager.js";
import { ssmRoutes } from "./routes/ssm.js";
...
export const awsPlugin: ServicePlugin = {
  ...
  register(app: Hono<AppEnv>, store: Store, webhooks: WebhookDispatcher, baseUrl: string, tokenMap?: TokenMap): void {
    ...
    app.use("/iam/*", awsAuth);
    app.use("/kms/*", awsAuth);
    app.use("/secretsmanager/*", awsAuth);
    app.use("/ssm/*", awsAuth);
    app.use("/s3/*", awsAuth);
    sqsRoutes(ctx);
    iamRoutes(ctx);
    // JSON-RPC services dispatch on X-Amz-Target at the root; register before S3
    // (whose wildcard GET/PUT paths would otherwise shadow the root handler).
    kmsRoutes(ctx);
    secretsManagerRoutes(ctx);
    ssmRoutes(ctx);
    s3Routes(ctx);
  },
