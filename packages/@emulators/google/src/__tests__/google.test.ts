...
function createTestApp() {
  ...
  seedFromConfig(store, base, {
    users: [
      { email: "testuser@agent-emulate.dev", name: "Test User" },
      { email: "consumer@gmail.com", name: "Consumer User" },
      { email: "workspaceuser@agent-emulate.dev", name: "Workspace User", hd: "override.io" },
    ],
    oauth_clients: [
      {
    ...
    messages: [
      {
        ...
        user_email: "testuser@agent-emulate.dev",
        from: "Support <support@agent-emulate.dev>",
      ...
      },
      {
        ...
        thread_id: "thread_support",
        user_email: "testuser@agent-emulate.dev",
        from: "testuser@agent-emulate.dev",
        to: "Support <support@agent-emulate.dev>",
      ...
      },
      {
        ...
        user_email: "testuser@agent-emulate.dev",
        from: "Billing <billing@agent-emulate.dev>",
        to: "testuser@agent-emulate.dev",
      ...
      },
      {
        ...
        user_email: "testuser@agent-emulate.dev",
        from: "Releases <release@agent-emulate.dev>",
        to: "testuser@agent-emulate.dev",
    ...
    calendar_events: [
      {
        ...
        attendees: [
          { email: "testuser@agent-emulate.dev", display_name: "Test User" },
          { email: "teammate@agent-emulate.dev", display_name: "Teammate" },
        ],
...
describe("Google plugin integration", () => {
  ...
  it("returns user info for a valid token", async () => {
    ...
    expect(body.email).toBe("testuser@agent-emulate.dev");
  ...
  it("returns message payloads in metadata and raw formats", async () => {
    ...
    expect(metadataBody.payload.headers).toEqual([
      { name: "From", value: "Releases <release@agent-emulate.dev>" },
  ...
  it("returns attachment parts and serves attachment bodies", async () => {
    const raw = buildRawMessage({
      from: "Contracts <contracts@agent-emulate.dev>",
      to: "testuser@agent-emulate.dev",
  ...
  it("tracks history entries after watch registration", async () => {
    ...
    const importRaw = buildRawMessage({
      from: "Alerts <alerts@agent-emulate.dev>",
      to: "testuser@agent-emulate.dev",
  ...
  it("lists settings resources and applies Gmail filters to matching messages", async () => {
    ...
    expect(sendAsBody.sendAs).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          sendAsEmail: "testuser@agent-emulate.dev",
    ...
    const createFilterRes = await jsonRequest(app, "/gmail/v1/users/me/settings/filters", {
      method: "POST",
      body: {
        criteria: { from: "billing@agent-emulate.dev" },
    ...
    expect(filter.criteria.from).toBe("billing@agent-emulate.dev");
  ...
  it("creates sent messages and appends them to existing threads", async () => {
    const raw = buildRawMessage({
      ...
      message_id: "<outbound-1@agent-emulate.dev>",
      in_reply_to: "<msg_support_1@emulate.google.local>",
      references: "<msg_support_1@emulate.google.local>",
    });
  ...
  it("derives, overrides, and omits the hd claim based on user config", async () => {
    ...
    const derived = await getIdTokenClaims("testuser@agent-emulate.dev");
    expect(derived.claims.hd).toBe("agent-emulate.dev");

    const overridden = await getIdTokenClaims("workspaceuser@agent-emulate.dev");
  ...
  it("issues an RS256 id_token verifiable against the JWKS endpoint", async () => {
    ...
    expect(payload.email).toBe("testuser@agent-emulate.dev");
  ...
  it("lists calendar resources, creates events, queries freebusy, and deletes events", async () => {
    ...
    const createEventRes = await jsonRequest(app, "/calendar/v3/calendars/primary/events", {
      ...
      body: {
        ...
        attendees: [{ email: "teammate@agent-emulate.dev", displayName: "Teammate" }],
        conferenceData: {
  ...
  it("introspects an issued access token (RFC 7662)", async () => {
    ...
    expect(active.username).toBe("testuser@agent-emulate.dev");
