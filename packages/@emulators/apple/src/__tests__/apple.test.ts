...
describe("Apple plugin integration", () => {
  ...
  it("introspects an issued access token and resolves userinfo (RFC 7662)", async () => {
    ...
    expect(introspect.username).toBe("testuser@agent-emulate.dev");
    ...
    expect(userinfo.email).toBe("testuser@agent-emulate.dev");
  ...
  it("sends user JSON blob only on first authorization per user+client pair", async () => {
    ...
    expect(parsed.email).toBe("testuser@agent-emulate.dev");
