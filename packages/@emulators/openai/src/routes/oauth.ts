...
import type { OpenAiUser } from "../entities.js";
...
function getPendingCodes(store: Store): Map<string, PendingCode> {
  let map = store.getData<Map<string, PendingCode>>("openai.oauth.pendingCodes");
  if (!map) {
    map = new Map();
    store.setData("openai.oauth.pendingCodes", map);
  }
  return map;
}
...
export function oauthRoutes({ app, store, tokenMap }: RouteContext): void {
  ...
  app.get("/oauth/authorize", (c) => {
    ...
    const clientsConfigured = os.oauthClients.all().length > 0;
    let clientName = "";
    if (clientsConfigured) {
      const client = os.oauthClients.findOneBy("client_id", client_id);
      if (!client) {
        return c.html(
    ...
    return c.html(renderCardPage("Sign in to OpenAI", subtitle, body, SERVICE_LABEL));
  });
  ...
  app.post("/oauth/token", async (c) => {
    ...
    // refresh_token grant: re-issue without a code (PKCE clients omit secret).
    if (grantType === "refresh_token") {
      const token = "sk-oat-" + randomBytes(24).toString("base64url");
      const user = os.users.all()[0];
      if (user && tokenMap) tokenMap.set(token, { login: user.email, id: user.id, scopes: [] });
      return c.json({
        access_token: token,
        token_type: "Bearer",
        expires_in: 3600,
        refresh_token: "sk-ort-" + randomBytes(24).toString("base64url"),
      });
    }
    ...
    if (clientsConfigured) {
      const client = os.oauthClients.findOneBy("client_id", bodyClientId);
      if (!client) {
        return c.json({ error: "invalid_client", error_description: "Unknown client_id." }, 401);
      }
      // Public/PKCE clients send no secret; confidential clients must match.
    ...
    const user = os.users.findOneBy("email", pending.email as OpenAiUser["email"]);
    if (!user) {
    ...
    debug("openai.oauth", `issued token for ${user.email}`);
    return c.json({
      access_token: token,
      token_type: "Bearer",
      expires_in: 3600,
  ...
  // ---------- Userinfo / current account ----------
  app.get("/oauth/userinfo", (c) => {
    const authUser = c.get("authUser");
    if (!authUser) return c.json({ error: { code: "unauthorized", message: "Authentication required" } }, 401);
    const user = os.users.findOneBy("email", authUser.login as OpenAiUser["email"]);
