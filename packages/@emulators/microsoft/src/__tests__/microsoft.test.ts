...
async function getAuthCode(
  ...
  const email = options.email ?? "testuser@agent-emulate.dev";
...
describe("Microsoft plugin integration", () => {
  ...
  it("completes full OAuth authorization_code flow", async () => {
    ...
    expect(claims.email).toBe("testuser@agent-emulate.dev");
    expect(claims.name).toBe("Test User");
    expect(claims.preferred_username).toBe("testuser@agent-emulate.dev");
  ...
  it("GET /oidc/userinfo returns user info when authenticated", async () => {
    ...
    expect(body.preferred_username).toBe("testuser@agent-emulate.dev");
  });
  ...
  it("GET /v1.0/me returns Graph-style user profile when authenticated", async () => {
    ...
    expect(body.mail).toBe("testuser@agent-emulate.dev");
    expect(body.userPrincipalName).toBe("testuser@agent-emulate.dev");
  ...
  it("seeds users and clients from config", () => {
    ...
    seedFromConfig(testStore, base, {
      users: [
        { email: "alice@agent-emulate.dev", name: "Alice Smith" },
        { email: "bob@agent-emulate.dev", name: "Bob Jones", tenant_id: "custom-tenant" },
      ],
      oauth_clients: [
        {
          client_id: "my-app",
    ...
    const alice = ms.users.findOneBy("email", "alice@agent-emulate.dev");
    expect(alice).toBeDefined();
    expect(alice!.name).toBe("Alice Smith");
    expect(alice!.preferred_username).toBe("alice@agent-emulate.dev");

    const bob = ms.users.findOneBy("email", "bob@agent-emulate.dev");
  ...
  it("GET /v1.0/users/:id returns user profile by oid", async () => {
    ...
    const user = ms.users.findOneBy("email", "testuser@agent-emulate.dev");
