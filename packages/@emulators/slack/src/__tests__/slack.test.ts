...
describe("Slack plugin - users", () => {
  ...
  it("looks up user by email", async () => {
    const res = await app.request(`${base}/api/users.lookupByEmail`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ email: "admin@agent-emulate.dev" }),
    });
    const body = (await res.json()) as any;
    expect(body.ok).toBe(true);
    expect(body.user.profile.email).toBe("admin@agent-emulate.dev");
  });
...
describe("Slack plugin - seedFromConfig", () => {
  it("seeds custom team, users, channels, and bots", () => {
    ...
    seedFromConfig(store, base, {
      ...
      users: [
        { name: "alice", real_name: "Alice Smith", email: "alice@agent-emulate.dev", is_admin: true },
        { name: "bob", email: "bob@agent-emulate.dev" },
      ],
      channels: [
