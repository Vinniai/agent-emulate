...
describe("Resend plugin - Emails", () => {
  ...
  it("POST /emails sends an email and returns id", async () => {
    const res = await app.request(`${base}/emails`, {
      ...
      headers: authHeaders(),
      body: JSON.stringify({
        from: "noreply@agent-emulate.dev",
        to: ["user@agent-emulate.dev"],
        subject: "Hello",
        html: "<p>World</p>",
      }),
    });
  ...
  it("POST /emails validates required fields", async () => {
    const res = await app.request(`${base}/emails`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ from: "noreply@agent-emulate.dev" }),
    });
  ...
  it("GET /emails/:id retrieves a sent email", async () => {
    ...
    expect(body.from).toBe("noreply@agent-emulate.dev");
  });
  ...
  it("GET /emails lists all emails", async () => {
    await app.request(`${base}/emails`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ from: "a@agent-emulate.dev", to: "c@agent-emulate.dev", subject: "S1" }),
    });
    await app.request(`${base}/emails`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ from: "a@agent-emulate.dev", to: "c@agent-emulate.dev", subject: "S2" }),
    });
  ...
  it("POST /emails/batch sends multiple emails", async () => {
    const res = await app.request(`${base}/emails/batch`, {
      ...
      body: JSON.stringify([
        { from: "a@agent-emulate.dev", to: "c@agent-emulate.dev", subject: "Batch 1" },
        { from: "a@agent-emulate.dev", to: "e@agent-emulate.dev", subject: "Batch 2" },
      ]),
    });
    expect(res.status).toBe(200);
  ...
  it("POST /emails/:id/cancel cancels a scheduled email", async () => {
    const sendRes = await app.request(`${base}/emails`, {
      ...
      headers: authHeaders(),
      body: JSON.stringify({
        from: "a@agent-emulate.dev",
        to: "c@agent-emulate.dev",
        subject: "Scheduled",
  ...
  it("POST /emails/:id/cancel fails for delivered email", async () => {
    const sendRes = await app.request(`${base}/emails`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ from: "a@agent-emulate.dev", to: "c@agent-emulate.dev", subject: "Sent" }),
    });
    const { id } = (await sendRes.json()) as { id: string };
...
describe("Resend plugin - Domains", () => {
  ...
  it("POST /domains creates a domain with DNS records", async () => {
    const res = await app.request(`${base}/domains`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ name: "agent-emulate.dev" }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
    expect(body.id).toBeDefined();
    expect(body.name).toBe("agent-emulate.dev");
...
describe("Resend plugin - Contacts & Audiences", () => {
  ...
  it("POST /audiences/:id/contacts creates a contact", async () => {
    ...
    const res = await app.request(`${base}/audiences/${audienceId}/contacts`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ email: "user@agent-emulate.dev", first_name: "Test" }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
    expect(body.email).toBe("user@agent-emulate.dev");
  });
  ...
  it("GET /audiences/:id/contacts lists contacts", async () => {
    ...
    await app.request(`${base}/audiences/${audienceId}/contacts`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ email: "a@agent-emulate.dev" }),
    });
  ...
  it("DELETE /audiences/:audience_id/contacts/:id deletes a contact", async () => {
    ...
    const ctRes = await app.request(`${base}/audiences/${audienceId}/contacts`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ email: "del@agent-emulate.dev" }),
    });
...
describe("Resend plugin - Inbox UI", () => {
  ...
  it("GET /inbox shows sent emails", async () => {
    await app.request(`${base}/emails`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ from: "a@agent-emulate.dev", to: "c@agent-emulate.dev", subject: "Test Subject" }),
    });
  ...
  it("GET /inbox/:id shows email detail", async () => {
    const sendRes = await app.request(`${base}/emails`, {
      ...
      headers: authHeaders(),
      body: JSON.stringify({
        from: "sender@agent-emulate.dev",
        to: "recipient@agent-emulate.dev",
        subject: "Detail Test",
        html: "<h1>Hello</h1>",
      }),
    });
    ...
    expect(html).toContain("sender@agent-emulate.dev");
    expect(html).toContain("recipient@agent-emulate.dev");
...
describe("Resend plugin - seedFromConfig", () => {
  it("seeds domains and contacts from config", () => {
    ...
    seedFromConfig(store, base, {
      domains: [{ name: "agent-emulate.dev" }],
      contacts: [{ email: "user@agent-emulate.dev", first_name: "Test", last_name: "User" }],
    });

    const rs = getResendStore(store);
    const domains = rs.domains.all();
    expect(domains.length).toBe(1);
    expect(domains[0].name).toBe("agent-emulate.dev");
    expect(domains[0].status).toBe("verified");

    const contacts = rs.contacts.all();
    expect(contacts.length).toBe(1);
    expect(contacts[0].email).toBe("user@agent-emulate.dev");
  });
});
