...
function createSeededApp() {
  ...
  seedFromConfig(store, base, {
    users: [
      { email_addresses: ["alice@agent-emulate.dev"], first_name: "Alice", last_name: "Smith", password: "alice123" },
      { email_addresses: ["bob@agent-emulate.dev"], first_name: "Bob", last_name: "Jones" },
    ],
    organizations: [
      {
        name: "Acme Corp",
        slug: "acme",
        members: [
          { email: "alice@agent-emulate.dev", role: "admin" },
          { email: "bob@agent-emulate.dev", role: "member" },
        ],
      },
    ],
    oauth_applications: [
...
describe("Clerk plugin integration", () => {
  ...
  describe("users API", () => {
    ...
    it("filters users by email_address", async () => {
      const res = await app.request(`${base}/v1/users?email_address=bob@agent-emulate.dev`, { headers: authHeaders() });
      expect(res.status).toBe(200);
      const body = (await res.json()) as { data: Array<{ first_name: string }>; total_count: number };
      expect(body.total_count).toBe(1);
    ...
    it("creates a user", async () => {
      const res = await app.request(`${base}/v1/users`, {
        ...
        headers: authHeaders(),
        body: JSON.stringify({
          email_address: ["new@agent-emulate.dev"],
          first_name: "New",
          last_name: "User",
          password: "secret",
        }),
      ...
      expect((body.email_addresses as Array<{ email_address: string }>)[0].email_address).toBe("new@agent-emulate.dev");
    });
  ...
  describe("email addresses API", () => {
    it("creates an email address", async () => {
      ...
      const res = await app.request(`${base}/v1/email_addresses`, {
        ...
        body: JSON.stringify({
          ...
          email_address: "alice2@agent-emulate.dev",
          verified: true,
        }),
      });
      expect(res.status).toBe(200);
      ...
      expect(body.email_address).toBe("alice2@agent-emulate.dev");
      expect((body.verification as Record<string, unknown>).status).toBe("verified");
    });

    it("gets an email address", async () => {
      const cs = getClerkStore(store);
      const email = cs.emailAddresses.all().find((e) => e.email_address === "alice@agent-emulate.dev")!;
      const res = await app.request(`${base}/v1/email_addresses/${email.email_id}`, { headers: authHeaders() });
      expect(res.status).toBe(200);
      const body = (await res.json()) as Record<string, unknown>;
      expect(body.email_address).toBe("alice@agent-emulate.dev");
    });
    ...
    it("deletes an email address", async () => {
      ...
      const createRes = await app.request(`${base}/v1/email_addresses`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ user_id: user.clerk_id, email_address: "delete-me@agent-emulate.dev" }),
      });
  ...
  describe("invitations API", () => {
    it("creates an invitation", async () => {
      ...
      const res = await app.request(`${base}/v1/organizations/${org.clerk_id}/invitations`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ email_address: "invite@agent-emulate.dev", role: "org:member" }),
      });
      expect(res.status).toBe(200);
      const body = (await res.json()) as Record<string, unknown>;
      expect(body.object).toBe("organization_invitation");
      expect(body.email_address).toBe("invite@agent-emulate.dev");
    ...
    it("lists invitations with status filter", async () => {
      ...
      await app.request(`${base}/v1/organizations/${org.clerk_id}/invitations`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ email_address: "inv1@agent-emulate.dev" }),
      });
    ...
    it("revokes an invitation", async () => {
      ...
      const createRes = await app.request(`${base}/v1/organizations/${org.clerk_id}/invitations`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ email_address: "revoke@agent-emulate.dev" }),
      });
    ...
    it("bulk creates invitations", async () => {
      ...
      const res = await app.request(`${base}/v1/organizations/${org.clerk_id}/invitations/bulk`, {
        ...
        headers: authHeaders(),
        body: JSON.stringify({
          email_addresses: ["bulk1@agent-emulate.dev", "bulk2@agent-emulate.dev"],
          role: "org:member",
        }),
      });
      expect(res.status).toBe(200);
  ...
  describe("seed config", () => {
    it("seeds users with email addresses", () => {
      ...
      expect(emails[0].email_address).toBe("alice@agent-emulate.dev");
    ...
    it("does not duplicate on re-seed", () => {
      ...
      seedFromConfig(store, base, {
        users: [{ email_addresses: ["alice@agent-emulate.dev"], first_name: "Alice" }],
      });
